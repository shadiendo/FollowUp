...
from tools import AppSettings, ValueInDatabase_read, ValueInDatabase_write, pickle2csv, searchPat_from_nameAndHospID, \
    howManyToFinsh, restore_from_theLastUnfinishedWork, messageBoxxx
        ...
        def searchPat():
            ...
            if IsThisPatFinished():
                ...
                if type(goal_index) != int:
                    messageBoxxx(self, '提示', goal_index)
                    return
                else:
                    DIV_.destroy()
                    ...
                    read_content()
                    messageBoxxx(self, '提示', f"找到该人了，序号为{goal_index}")
                    name_input.set("")
                    hosId_input.set("")
            else:
                messageBoxxx(self, '请先点击【完成】', "为什么要先点击完成？为了减少数据丢失")
                return
        ...
        def restorationTheWork():
            ...
            if IsThisPatFinished():
                ...
                if type(getIndex) != int:
                    ...
                    scr_note.delete(1.0, END)
                    read_content()
                    messageBoxxx(self, '提示', f"从该病人开始吧！序号为{getIndex}")
                    name_input.set("")
                    hosId_input.set("")
            else:
                messageBoxxx(self, '请先点击【完成】', "为什么要先点击完成？为了减少数据丢失")
